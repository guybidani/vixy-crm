...
export default function PageSkeleton() {
  return (
    <div className="min-h-screen bg-[#F6F7FB] flex" dir="rtl" aria-busy="true" aria-label="טוען...">
      {/* Sidebar shimmer */}
      <div className="hidden md:flex flex-col w-[220px] flex-shrink-0 bg-white border-l border-[#E6E9EF] p-3 gap-2">
        {/* Logo area */}
        <div className="h-10 w-32 bg-[#F6F7FB] rounded-xl animate-pulse mb-3" />
        {/* Nav items */}
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="h-9 rounded-[4px] animate-pulse bg-[#F6F7FB]"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-14 bg-white border-b border-[#E6E9EF] flex items-center px-6 gap-4">
          <div className="h-8 w-48 bg-[#F6F7FB] rounded-[4px] animate-pulse" />
          <div className="flex-1" />
          <div className="h-8 w-8 rounded-full bg-[#F6F7FB] animate-pulse" />
          <div className="h-8 w-8 rounded-full bg-[#F6F7FB] animate-pulse" />
        </div>
        ...
        <div className="p-6 flex flex-col gap-5 flex-1">
          {/* Page title */}
          <div className="h-8 w-56 bg-[#F6F7FB] rounded-xl animate-pulse" />
